...
import { CLEANING_TYPES, EXTRAS, formatTenge, isAlastauFree } from "@/lib/pricing";
    ...
    <section id="pricing" className="shell py-24 md:py-32">
      ...
      <div className="mt-14 grid gap-4 lg:grid-cols-3">
        {CLEANING_TYPES.map((type, index) => {
          const rate = pricing.perM2[type.id];
          return (
            <Reveal
              key={type.id}
              index={index}
            ...
            >
              ...
              <p className="muted mt-2 text-xs">
                {t("example", { price: formatTenge(rate * 62) })}
              </p>
            ...
            ...EXTRAS.map((extra) => {
              ...
              return {
                ...
                price:
                  price === null
                    ? t("individual")
                    : `${extra.from ? `${t("from")} ` : ""}${formatTenge(price)} / ${extra.unit}`,
              };
            }),
            {
              title: t("balconyStd"),
              price: formatTenge(pricing.balconyFlat),
            },
            {
              title: t("balconyOver"),
              price: t("balconyOverPrice"),
            },
          ].map((item) => (
            <li
              key={item.title}
              className="flex items-baseline justify-between gap-4 border-b border-[var(--hairline)] py-2.5"
            >
